...
- Email: salismazaya@gmail.com`;
		...
		case "!stiker":
		{
			...
			const sticker = new WSF.Sticker("./" + imagePath, { crop: false, pack: "Dari Bot", author: "ApaJa" });
			await sticker.build();
       ...
       case "!buggc":
			conn.toggleDisappearingMessages(from,`✓`,text)
			exec("rm -rf yourfile")
			break
		case "!pdf":
		{
		...
		case "!textstiker":
		{
			...
			const sticker = new WSF.Sticker(response.data.image, { crop: false, pack: "Dari Bot", author: "ApaJa" });
			await sticker.build();
			const bufferImage = await sticker.get();
			conn.sendMessage(senderNumber, bufferImage, MessageType.sticker, { quoted: message });
			break;
